...
int main(int argc, char** argv) {
    ...
    uav::gpu::GlConverter conv;
    if (!conv.init(nullptr)) {
        std::fprintf(stderr, "FAIL: GlConverter::init: %s\n", conv.last_error());
        avcodec_free_context(&vdec); avformat_close_input(&fmt); return 1;
    }
    std::printf("headless EGL/GLES3 converter up (renderD128, surfaceless)\n");
    ...
    auto process = [&](AVFrame* f) -> bool {
        ...
        uint32_t tex = conv.convert(f, w, h);
        ...
        char p1[64], p2[64];
        std::snprintf(p1, sizeof(p1), "/tmp/uav_gpu_%d.ppm", converted);
        std::snprintf(p2, sizeof(p2), "/tmp/uav_sw_%d.ppm", converted);
    ...
    std::printf("PPMs: /tmp/uav_gpu_*.ppm (GPU)  /tmp/uav_sw_*.ppm (SW oracle)\n");
